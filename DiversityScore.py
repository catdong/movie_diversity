...
def diversityProfitCorrelation(graph):
	...
	for node in graph.nodes():
		node_type = graph.node[node]["type"]
		if node_type == "MOVIE":
			data = profitStats(graph, node)
			if data[0] != None and data[1] != None and data[2] != None: 
				race_scores.append(data[0])
				gender_scores.append(data[1])
				profit_ratios.append(data[2])

	print "race scores"
	A = np.asarray(race_scores).reshape(len(race_scores), 1)

	A = np.concatenate(np.asmatrix(race_scores).T, np.asmatrix(profit_ratios).T)
	B = np.concatenate(np.asmatrix(gender_scores).T, np.asmatrix(profit_ratios).T)

	print A

	return (np.corrcoef(A), np.corrcoef(B))
